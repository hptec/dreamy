...
## 迭代 2：2026-05-30 — 后台权限管理与管理员管理

### 产品定位增量

为 Dreamy 运营管理后台（portal-admin）新增完整的 RBAC 权限体系，将系统设置拆分为独立菜单项，支持菜单级别的权限管控。

### 目标用户

后台运营团队（超管 / 商品运营 / 订单客服 / 内容编辑 / 数据分析），本轮为其提供精细化的权限分配能力。

### 核心功能（Must Have）

1. **管理员管理**（独立菜单）
   - 管理员列表：表格展示（姓名/邮箱/角色/最近登录/状态），支持搜索和筛选
   - 新增管理员：弹窗表单（姓名/邮箱/密码/角色分配/状态）
   - 编辑管理员：弹窗表单（姓名/角色/状态，邮箱不可改）
   - 删除管理员：确认弹窗后删除（不可删除自己）
   - 禁用/启用：行内 Toggle 切换
   - 重置密码：行内按钮（前端演示，无实际邮件发送）

2. **角色权限管理**（独立菜单）
   - 角色列表：卡片或表格展示现有角色（超管/商品运营/订单客服/内容编辑/数据分析）
   - 权限矩阵：角色 × 后台菜单项 的复选框矩阵
     - 菜单项覆盖所有侧边栏菜单分组和子项
     - 二元权限：勾选 = 菜单可见 + 路由可访问，未勾选 = 菜单隐藏 + URL 拦截
   - 支持新增/编辑/删除自定义角色

3. **操作日志**（独立菜单，从 Settings Tab 升级）
   - 日志列表：时间/操作人/操作类型/操作对象/IP
   - 支持按操作人、操作类型、时间范围筛选
   - 点击行可查看操作详情（变更前后对比）
   - 日志不可删除（只读）

4. **菜单重组**
   - 新增侧边栏分组「系统管理」，下设三个菜单项：
     - 管理员管理（/system/admins）
     - 角色权限（/system/roles）
     - 操作日志（/system/logs）
   - 移除原「发布与系统」分组下的「系统设置」菜单项

5. **权限管控生效**
   - 路由守卫：无权限菜单对应的路由拦截，重定向到 Dashboard
   - 侧边栏：根据当前登录管理员的角色动态渲染菜单
   - 当前 mock 默认超管账号拥有全部权限

### 权限粒度假定

- 菜单级别：一个菜单项（路由）为一个权限点
- 无数据级别权限（同角色管理员看到相同数据）
- 无操作级别权限（有菜单权限即可查看和操作）

### 排除项

- 细粒度数据权限（按品类/区域限制数据范围）
- 密码重置邮件发送（仅前端按钮）
- IP 白名单/登录二次验证等高级安全功能
