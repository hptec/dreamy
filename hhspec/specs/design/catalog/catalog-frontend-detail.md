...
> API 契约消费：catalog-api-detail.md E-CAT-01~35；错误处理按 error-strategy 前端呈现约定（admin 按 code toast/表单分发，store 按 code 映射 next-intl 字典）。
...
| portal-admin/src/views/Categories.vue（59 行） | **旧「品类与主题」版本**，与迭代 4 三层模型不符 | 按原型 618 行版「复制+适配」迁移（三层树 + 属性 delta 抽屉 + 标签维度/标签 tab），数据层接 API |
...
| PAGE-CAT-A02 | /products/new 与 /products/:id/edit | ProductEdit.vue | /products | E-CAT-09/10/11、E-CAT-35 presign、E-CAT-15（品类级联）、E-CAT-19/23（属性配置）、E-CAT-27/31（标签选择器） |
| PAGE-CAT-A03 | /categories | Categories.vue | /categories | E-CAT-15/16/17/18、E-CAT-27/28/29/30（维度）、E-CAT-31/32/33/34（标签）、E-CAT-19（绑定属性集下拉）、E-CAT-35（标签封面上传） |
...
listTagDimensions() / createTagDimension / updateTagDimension / deleteTagDimension
listTags(dimensionId?) / createTag / updateTag / deleteTag
...
- STORE-CAT-A04 `useTagsStore`：{ dimensions[], tagsByDim:Map, loading, fetchDimensions(), fetchTags(dimId?), saveDimension/removeDimension/saveTag/removeTag }；派生 `tagsByDimension(dimId)`（ProductEdit 标签选择器数据源，契约 CustomTag 合并声明的消费侧落点）
- STORE-CAT-A05 上传复用：`useUpload()` composable —— uploadViaPresign 封装 + 进度态 + 502501 降级提示（"对象存储暂不可用，可先保存其他字段"）

### A.4 组件树（COMP-CAT-A）

- COMP-CAT-A01 `Products.vue`（PAGE-CAT-A01）：PageHeader + 筛选栏（search 防抖 300ms / category 级联 / status）+ 更多筛选面板（productType 多选、库存档、flags、价格区间、tagIds——**本期价格区间与 tagIds 透传 API**（price_min/price_max/tag_id 仅消费端契约有，admin 契约无 → productType/库存档/flags/tagIds 为前端内存过滤当前页，与原型行为一致并在 tooltip 标注「当前页过滤」）+ 表格（Toggle status→E-CAT-13、Toggle isNew/recommend→E-CAT-14、sort 输入 blur 提交→E-CAT-14、删除→确认弹窗→E-CAT-12，409509 toast"已发布商品需先下架"）+ Pagination（服务端分页）+ EmptyState/骨架行 loading（复用既有组件风格）
- COMP-CAT-A02 `ProductEdit.vue`（PAGE-CAT-A02，原型锚点布局迁移）：
  ...
  - `sec-basic`：name/slug/subtitle/categoryId 两级级联（STORE-CAT-A02.cascadeOptions）/productType/标签选择器（STORE-CAT-A04.tagsByDimension，按维度分组多选 chip）
...
- COMP-CAT-A03 `Categories.vue`（PAGE-CAT-A03，原型三层模型迁移）：tab=品类树/标签维度；品类树面板（三层缩进 + Bars3 拖拽排序→保存触发 update sort + count 徽章 + 编辑抽屉 saveDrawer：根=改名/绑属性集，子=attr_overrides delta 三态覆盖 + 三语 name tab）+ 删除按钮 `:disabled=!canDelete`（product_count===0 且无子级前端预判；后端 409502 兜底 toast）；标签 tab（维度列表 confirmAddDim/编辑/删除——删除 409506 toast"维度下仍有标签"；标签卡片 confirmAddTag（name trim 必填）/封面上传（scope=tag）/Toggle enabled↔status/删除）
...
- FORM-CAT-A05 标签维度删除：409506 → toast 引导先清空标签（真实端较原型 removeDim 级联收紧，见 E-CAT-30 设计说明）
...
fetchStoreTags(locale, dimensionId?)      GET /api/store/tags
```
...
| PAGE-CAT-S04 | /（首页推荐位区块） | RSC + ISR | `revalidate = 300` + on-demand（product_flags_changed / 销量回写失效仅清后端缓存，页面靠 TTL）；区块：new_arrivals / best_sellers / shop_by_color（色板 tag 来自 E-CAT-07） | E-CAT-03 + E-CAT-07 |
...
|---|---|---|
| portal-admin/src/views/Products.vue | portal-admin/src/views/Products.vue | **layout-keep + data-swap**：布局/列/Toggle 形态不变；mock→API、内存筛选→服务端分页；「更多筛选」面板按原型补齐，价格/标签外的高级项为当前页过滤（标注） |
| portal-admin/src/views/ProductEdit.vue | portal-admin/src/views/ProductEdit.vue | **copy-adapt**：真实页为旧版壳，按原型锚点布局整体迁移后仅动数据层；三语 tab/标签选择器/属性显隐 与原型同构 |
| portal-admin/src/views/Categories.vue | portal-admin/src/views/Categories.vue | **copy-adapt**：真实页为旧「品类与主题」版，按原型三层模型+标签维度版迁移；删除维度行为收紧为 409506 引导（显式偏离，原型为级联删除——防误删营销数据，决策记录于 E-CAT-30） |
