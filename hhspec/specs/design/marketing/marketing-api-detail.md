...
- **缓存（BE-DIM-8）**：消费端内容/闪购读端点按缓存矩阵走 JetCache 两级 + CDN s-maxage；后台写操作提交后递增 Redis 共享 family 代际，所有实例立即切换到 `v{generation}` 命名空间，再发布 MQ `content.invalidated` 驱动 CDN 失效（FLOW-P03）；coupons/validate、newsletter、contact 不缓存；后台端点一律不缓存。该方案不依赖单进程 key registry，旧代 key 由 TTL 自然回收。key/TTL 详见 marketing-data-detail.md 第 8 节（CACHE-MKT-*）。
- **营销状态枚举线格式**：query、请求体、响应体统一使用 IntEnum 数字值；列表查询省略 status 表示全部，不传字符串 `all`。`ContentStatus(Banner/Blog)={1:DRAFT,2:PUBLISHED,3:ARCHIVED}`、`CouponStatus={1:DRAFT,2:SCHEDULED,3:ACTIVE,4:EXPIRING,5:EXPIRED}`、`FlashSaleStatus={1:DRAFT,2:SCHEDULED,3:ACTIVE,4:ENDED}`、`PublishStatus(Wedding/Lookbook/Guide)={1:DRAFT,2:PUBLISHED}`。语义名只用于说明和前端标签，不作为 JSON 字符串传输。
...
- **DEC-MKT-2 Banner 到期口径**：消费端读路径（E-MKT-01）恒按投放窗口过滤（status=2/PUBLISHED 且 now∈[start_time,end_time]，空窗端开放），FLOW-P15 权威口径「now>end → 移出（**状态不变**，读路径按窗口过滤）」；SCHED-MKT-01 扫描到窗口边界穿越（进入/移出投放）仅触发缓存失效链，**不翻转 Banner status**。state-machine `schedule_expire`（2/PUBLISHED→3/ARCHIVED）迁移由「窗口过滤造成消费端下线」语义承载，管理端列表以「已过窗」派生标识展示；手动下线仍走 E-MKT-25（2→3）。归因：避免定时任务与管理员手动 republish 的状态写竞争，且与 E-MKT-01 契约描述（按窗口过滤）一致。
- **DEC-MKT-3 Coupon/FlashSale 状态翻转**：与 Banner 不同，Coupon（2/SCHEDULED→3/ACTIVE→4/EXPIRING→5/EXPIRED）与 FlashSale（2/SCHEDULED→3/ACTIVE→4/ENDED）状态列由 SCHED-MKT-01 **实际翻转落库**（data-flow FLOW-P15 显式 UPDATE 语义；s-760/s-761），FlashSale 翻转触发失效链，Coupon 翻转不发 MQ（无消费端缓存面，见 E-MKT-14 注记）。expiring 阈值 = end_at − now ≤ 72h（配置项 `dreamy.marketing.coupon-expiring-hours` 缺省 72）。
...
- V-MKT-001 position 可选 ∈ {1=HERO, 2=FEATURED, 3=TOPBAR}（必须为整数；枚举外 → 422 `422704` fields.position=invalid_enum）
- V-MKT-002 locale ∈ {en, es, fr}，缺省 en（枚举外 → 422 `422704` fields.locale=invalid_enum）

**业务步骤**:
- STEP-MKT-01 查 JetCache `marketing:banners:v{generation}:{position-code|all}:{locale}`（position-code 为 1/2/3；generation 取 Redis 共享代际，TTL 300s）命中即返回
- STEP-MKT-02 `SELECT banner WHERE status=2 AND (start_time IS NULL OR start_time<=now) AND (end_time IS NULL OR end_time>now)`，position 给定则按 TINYINT 码过滤，ORDER BY sort ASC, id ASC（投放窗口过滤=DEC-MKT-2 权威读口径）；返回全部匹配项，position=1/HERO 不只取首条
- STEP-MKT-03 locale=es/fr → 批查 banner_translation(banner_id IN, locale)，命中字段覆盖 title/subtitle/cta_text/cta_text_secondary，缺翻译回退 EN 主表列；cta_link/cta_link_secondary 为语言无关链接，直接取主表（DEC-MKT-1/KD-14）
- STEP-MKT-04 装配 StoreBanner（id/name/image_url/position(IntEnum 1/2/3)/sort/title/subtitle/cta_text/cta_link/cta_text_secondary/cta_link_secondary，**不暴露** clicks/status/start_time/end_time）→ 写当次 lookup 捕获的代际缓存（空集同样缓存）→ `Cache-Control: s-maxage=300`
...
- STEP-MKT-02 `SELECT blog_post WHERE status=2`（category 给定则 =），ORDER BY published_at DESC, id DESC，分页 LIMIT/OFFSET（IDX-MKT-005）
...
- STEP-MKT-02 `SELECT blog_post WHERE slug=? AND status=2`；不存在/未发布 → 写 null 缓存 60s（穿透保护，BE-DIM-8）→ 404 `404701`
...
- STEP-MKT-02 `SELECT real_wedding WHERE status=2` ORDER BY wedding_date DESC, id DESC，分页
...
- STEP-MKT-02 `SELECT real_wedding WHERE id=? AND status=2`；不存在/status=1 → null 缓存 → 404 `404701`
...
- STEP-MKT-02 `SELECT lookbook WHERE status=2` ORDER BY id DESC
...
- STEP-MKT-02 `SELECT lookbook WHERE id=? AND status=2`；不存在/status=1 → null 缓存 → 404 `404701`
...
- STEP-MKT-02 `SELECT guide WHERE status=2` ORDER BY phase ASC, id ASC（phase 文本 'Phase 1'..，字典序即阶段序）
...
- STEP-MKT-02 `SELECT flash_sale WHERE status=3` ORDER BY end_at ASC（到期翻转由 SCHED-MKT-01 保证，读路径不再叠窗口过滤——以状态列为准，DEC-MKT-3）
...
- STEP-MKT-02 可用性判定（顺序固定，首个命中即返回 200 valid=false + reason_code）：
  1. 不存在 / status=1(DRAFT) / （status=2(SCHEDULED) 或 start_at>now）→ reason_code=`422701`（未开始）
  2. status=5(EXPIRED) 或 (end_at 非空且 now>end_at) → reason_code=`422701`（已过期；SCHED 翻转与实时判定双保险）
...
- V-MKT-017 status 可选 IntEnum ∈ {1=DRAFT,2=SCHEDULED,3=ACTIVE,4=EXPIRING,5=EXPIRED}；省略表示全部，字符串 `all`/枚举外值 → 422 `422704`
- V-MKT-018 search ≤64（trim 后空 → 视为未提供）

**业务步骤**:
- STEP-MKT-01 组装条件：status 参数非空时按数字 key 过滤；search → `(code LIKE %s% OR name LIKE %s%)`
...
- V-MKT-026 status 必填 IntEnum ∈ {1=DRAFT,2=SCHEDULED,3=ACTIVE,4=EXPIRING,5=EXPIRED} 且与时间窗一致：2 要求 start_at 非空且 > now（bs-786）；3 要求 (start_at 空或 ≤now) 且 (end_at 空或 >now)（bs-787 同族）；4/5 不可作为创建态（违反 → 422 `422704` fields.status=inconsistent_with_window）
...
- 复用 V-MKT-019 ~ V-MKT-027（code 唯一性排除自身；status 时间窗一致性放宽：编辑时允许保持 DB 当前 status∈{4=EXPIRING,5=EXPIRED} 原值不动，仅禁止**改入**该两态）
...
- STEP-MKT-02 状态机 guard：DB status ∈ {3=ACTIVE,4=EXPIRING} 且提交 code ≠ DB code → 409 `409703` CONTENT_STATE_INVALID（已上线券改码会使用户手中券码失效）
...
- STEP-MKT-02 状态机 guard①：status ∉ {1=DRAFT,5=EXPIRED} → 409 `409703`（status=2/3/4 须先到期）
...
**入参**: query `{ status? }`
- V-MKT-030 status 可选 IntEnum ∈ {1=DRAFT,2=SCHEDULED,3=ACTIVE,4=ENDED}；省略表示全部，字符串 `all`/枚举外值 → 422 `422704`

**业务步骤**:
- STEP-MKT-01 `SELECT flash_sale`（status 参数非空时按数字 key 过滤）ORDER BY start_at DESC, id DESC
...
- V-MKT-034 status 必填 IntEnum ∈ {1=DRAFT,2=SCHEDULED,3=ACTIVE,4=ENDED} 且与时间窗一致：2 要求 start_at>now（bs-798）；3 要求 start_at≤now<end_at；**4 不可作为创建态**（→ 422 `422704` fields.status=inconsistent_with_window）
...
- STEP-MKT-03 提交后（status=3/ACTIVE 时）：@CacheInvalidate `marketing:flash:*` → MQ publish `content.invalidated {event_id, type:flash_sale_changed, locales:[en,es,fr]}`（status=1/2 无消费端可见性变化，不发——SCHED 翻转时再发）
...
- V-MKT-037 id 正整数（非法视同不存在 → 404 `404703`）；复用 V-MKT-031~036（status 一致性放宽同 E-MKT-15：不可改入 4/ENDED，该状态由 SCHED 专有）

**业务步骤（单事务 TX-MKT-005）**:
- STEP-MKT-01 `SELECT flash_sale WHERE id=?`；不存在 → 404 `404703`
- STEP-MKT-02 状态机 guard：DB status=4(ENDED) → 409 `409703`（flash_sale_lifecycle ENDED 为终态）
...
- STEP-MKT-02 状态机 guard：status ≠ 1/DRAFT → 409 `409703`（status=2/3/4 保留运营痕迹）
...
## 6. ADMIN Banner 端点（RBAC `/banners`，不缓存；写操作触发 BANNERS family 共享代际失效链）

### E-MKT-21 listAdminBanners — GET /api/admin/banners

**入参**: query `{ position? }`
- V-MKT-038 position 可选 IntEnum ∈ {1=HERO, 2=FEATURED, 3=TOPBAR}；非法值 → 422 `422704`
...
**错误映射**: 403 `40300` / 422 `422704` / 500 `50000`
...
- V-MKT-040 position 必填 IntEnum ∈ {1=HERO, 2=FEATURED, 3=TOPBAR}（bs-032/398）
...
- V-MKT-042 status 必填 IntEnum ∈ {1=DRAFT, 2=PUBLISHED, 3=ARCHIVED}；**创建态仅允许 1/2**（3 → 422 `422704` fields.status=invalid_initial；banner_lifecycle initial=1/DRAFT，publish guard name/image_url 非空由必填校验天然满足，bs-733）
- V-MKT-043 sort 必填 int ≥ 0（bs-035）
- V-MKT-044 translations[] locale ∈ {es, fr} 不重复；title ≤255 / subtitle ≤255 / cta_text ≤64 / cta_text_secondary ≤64；EN 可选列为 title/subtitle/cta_text/cta_link(≤512)/cta_text_secondary(≤64)/cta_link_secondary(≤512)（DEC-MKT-1/KD-14）

**业务步骤（单事务 TX-MKT-007）**:
- STEP-MKT-01 INSERT banner（clicks=0 初始化）+ banner_translation 批插
- STEP-MKT-02 INSERT operation_log(action=创建Banner)
- STEP-MKT-03 提交后（status=2/PUBLISHED）：`invalidateFamily(BANNERS)` 递增 Redis 共享代际 → MQ `content.invalidated {event_id, type:banner_changed, locales:[en,es,fr]}` → 消费者 revalidate `/` ×3 locale + purge（FLOW-P03）
...
- STEP-MKT-02 status 变更时校验迁移合法性（banner_lifecycle）：1→2 / 2→3 / 3→2 合法；其余迁移 → 409 `409703`（同 E-MKT-25 guard，整单保存复用）
- STEP-MKT-03 UPDATE banner（SET 不含 clicks）+ banner_translation 整单覆盖
- STEP-MKT-04 INSERT operation_log(action=编辑Banner)
- STEP-MKT-05 提交后：`invalidateFamily(BANNERS)` 递增 Redis 共享代际 → MQ `content.invalidated {type:banner_changed}`（契约「写成功触发失效链」，status=1 的编辑亦发——sort/窗口变更影响在投放清单）
...
- STEP-MKT-02 物理删除 banner + banner_translation（status=1/2/3 均可物理删除，状态机无 guard）；INSERT operation_log(action=删除Banner)
- STEP-MKT-03 提交后：`invalidateFamily(BANNERS)` 递增 Redis 共享代际 → MQ `content.invalidated {type:banner_changed}`
...
- V-MKT-047 status 必填 IntEnum ∈ {1=DRAFT, 2=PUBLISHED, 3=ARCHIVED}

**业务步骤（单事务 TX-MKT-010）**:
- STEP-MKT-01 不存在 → 404 `404701`
- STEP-MKT-02 幂等：目标态=当前态 → 直接返回当前行（不写审计不发事件）
- STEP-MKT-03 迁移 guard（banner_lifecycle）：合法集 {1→2（Toggle 首次上线）, 2→3（take_offline）, 3→2（republish）}；其余（2→1 / 3→1 / 1→3）→ 409 `409703`
- STEP-MKT-04 `UPDATE banner SET status=?`；INSERT operation_log(action=编辑Banner, changes={status: from→to})
- STEP-MKT-05 提交后：`invalidateFamily(BANNERS)` 递增 Redis 共享代际 → MQ `content.invalidated {type:banner_changed}` → revalidate `/` ×3 + purge

**出参**: 200 Banner
**错误映射**: 403 `40300` / 404 `404701` / 409 `409703` / 422 `422704` / 500 `50000`
...
**入参**: query `{ page?, page_size?, status?, search? }`
- V-MKT-048 page/page_size 同 V-MKT-004；status 可选 IntEnum ∈ {1=DRAFT,2=PUBLISHED,3=ARCHIVED}，省略表示全部，字符串 `all`/枚举外值 → 422 `422704`
...
- V-MKT-052 slug 可选，匹配 `^[a-z0-9-]+$` 且 ≤128；**status=2/PUBLISHED 时必填**（发布生成静态文章页路径，缺 → 422 `422704` fields.slug=required_for_publish）；唯一 → 409 `409702`
- V-MKT-053 status 必填 IntEnum ∈ {1=DRAFT,2=PUBLISHED,3=ARCHIVED}；创建态仅 1/2（3 → 422 `422704` fields.status=invalid_initial）
...
- STEP-MKT-02 INSERT blog_post（status=2 时记 published_at=now；views=0 初始化）+ blog_post_translation 批插
- STEP-MKT-03 INSERT operation_log(action=创建文章)
- STEP-MKT-04 提交后（status=2/PUBLISHED）：@CacheInvalidate `marketing:blogs:*` + `marketing:blog:{slug}:*` → MQ `content.invalidated {type:blog_changed, slug, locales}` → revalidate `/blog` + `/blog/{slug}` ×3 locale + purge
...
**业务步骤（单事务 TX-MKT-012）**:
- STEP-MKT-01 `SELECT blog_post WHERE id=?`；不存在 → 404 `404701`
- STEP-MKT-02 status 变更时迁移 guard（blog_post_lifecycle）：1→2（记 published_at）/2→3/3→2 合法；其余 → 409 `409703`
- STEP-MKT-03 slug 变更查重（排除自身）→ 409 `409702`
- STEP-MKT-04 UPDATE blog_post（SET 不含 views；published_at 仅在 1→2 时写入，已发布文章编辑不刷新发布时间）+ translation 整单覆盖
- STEP-MKT-05 INSERT operation_log(action=编辑文章)
- STEP-MKT-06 提交后（DB 或目标 status=2，或 2→3 下线）：@CacheInvalidate `marketing:blogs:*` + `marketing:blog:{slug}:*`（新旧 slug 都失效）→ MQ `content.invalidated {type:blog_changed, slug, old_slug?}` → revalidate + purge（秒级失效，s-758）
...
- STEP-MKT-03 提交后（原 status=2/PUBLISHED）：失效 `marketing:blogs:*` + `marketing:blog:{slug}:*` → MQ `content.invalidated {type:blog_changed, slug}`（文章页 revalidate 后 404701，列表移除）
...
**入参**: path id；body `{ status! }`
- V-MKT-057 status 必填 IntEnum ∈ {1=DRAFT,2=PUBLISHED,3=ARCHIVED}

**业务步骤（单事务 TX-MKT-014）**:
- STEP-MKT-01 不存在 → 404 `404701`
- STEP-MKT-02 幂等：目标态=当前态 → 直接返回（不写审计不发事件）
- STEP-MKT-03 迁移 guard：1→2（**slug 必须非空** → 否则 422 `422704` fields.slug=required_for_publish；记 published_at=now）/2→3（unpublish）/3→2（republish，published_at 保持原值）合法；其余（1→3 / 2→1 / 3→1）→ 409 `409703`（bs-739~743）
...
- V-MKT-058 page/page_size 同 V-MKT-004；status 可选 PublishStatus ∈ {1=DRAFT,2=PUBLISHED}，省略表示全部，字符串 `all`/枚举外值 → 422 `422704`
...
- V-MKT-061 status 必填 PublishStatus ∈ {1=DRAFT,2=PUBLISHED}（bs-230/505）
...
- STEP-MKT-03 提交后（status=2/PUBLISHED）：@CacheInvalidate `marketing:weddings:*` + `marketing:wedding:*` → MQ `content.invalidated {type:wedding_changed, id, locales}` → revalidate `/real-weddings`、`/real-weddings/{id}`、`/` ×3 locale + purge
...
**入参**: path id；body `{ status! }`
- V-MKT-065 status 必填 PublishStatus ∈ {1=DRAFT,2=PUBLISHED}

**业务步骤（单事务 TX-MKT-018）**:
- STEP-MKT-01 不存在 → 404 `404701`
- STEP-MKT-02 幂等：同态 → 直接返回（bs-888/889 重复 publish 仅一次副作用）
- STEP-MKT-03 status 1→2（publish）/2→1（unpublish）双向合法（real_wedding_publish 两迁移；二态机无非法迁移，409 仅同请求并发冲突由幂等短路吸收，bs-589/590）
...
**入参**: path id；body `{ status! }`
- V-MKT-073 status 必填 PublishStatus ∈ {1=DRAFT,2=PUBLISHED}
...
**入参**: query `{ status? }`
- V-MKT-074 status 可选 PublishStatus ∈ {1=DRAFT,2=PUBLISHED}，省略表示全部，字符串 `all`/枚举外值 → 422 `422704`
...
- V-MKT-079 status 必填 PublishStatus ∈ {1=DRAFT,2=PUBLISHED}（bs-223/499）
...
**入参**: path id；body `{ status! }`
- V-MKT-082 status 必填 PublishStatus ∈ {1=DRAFT,2=PUBLISHED}
