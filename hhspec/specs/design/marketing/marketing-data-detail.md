...
- **枚举落地**：营销状态均以 `TINYINT + IntEnum` 落库并以数字线格式传输：ContentStatus 1/2/3、CouponStatus 1..5、FlashSaleStatus 1..4、PublishStatus 1/2；BannerPosition 同为 1/2/3。语义名仅用于代码枚举常量与 UI 标签，列表查询省略 status 表示全部，不传字符串 `all`。
...
| 实体 | 表名 | 要点 |
|---|---|---|
| Banner | banner | position IntEnum 1/2/3；投放窗口 start_time/end_time（读路径过滤，DEC-MKT-2）；status IntEnum 1/2/3；sort；clicks 只读统计；EN 文案列 title/subtitle/cta_text/cta_text_secondary 与双 CTA 链接 |
| BannerTranslation | banner_translation | uk(banner_id, locale)；locale ∈ {es,fr} |
| BlogPost | blog_post | slug 唯一（发布必填）；ContentStatus 1/2/3；published_at（首次发布记）；views 只读近似计数（DEC-MKT-6） |
| BlogPostTranslation | blog_post_translation | uk(blog_post_id, locale)；title/excerpt/body/seo_title/seo_description |
| RealWedding | real_wedding | couple 必填；PublishStatus 1/2；EN 文案列 title/story |
...
| Lookbook | lookbook | title 必填；PublishStatus 1/2；EN 文案列 description |
...
- RM-MKT-001 `listStoreActive(position?) -> List<Banner>` —— status=2 AND 窗口谓词（E-MKT-01；IDX-MKT-007）ORDER BY sort, id
...
- RM-MKT-020 `pageStorePublished(category?, page) -> Page<BlogPost>` —— status=2 ORDER BY published_at DESC（IDX-MKT-005）
...
- RM-MKT-040 `pageStorePublished(page)` —— status=2 ORDER BY wedding_date DESC（IDX-MKT-008）
...
- RM-MKT-120 `listStoreActive() -> List<FlashSale>` —— status=3 ORDER BY end_at（IDX-MKT-003）
...
- MAP-MKT-001 Banner→StoreBanner：id/name/image_url/position(IntEnum 1/2/3)/sort + title/subtitle/cta_text/cta_link/cta_text_secondary/cta_link_secondary（locale 解析后扁平输出）；**不暴露** status/start_time/end_time/clicks
- MAP-MKT-002 Banner→Banner DTO（admin）：BannerUpsert 全字段回显（position/status 均为 IntEnum 1/2/3）+ id/clicks + translations 三语 tab 原样（不回退合并）+ EN 文案列（DEC-MKT-1 可选字段）
...
- MAP-MKT-013 枚举：BannerPosition/ContentStatus/CouponStatus/FlashSaleStatus/PublishStatus 均按各自 IntEnum 数字 key 双向映射；禁止把 HERO/PUBLISHED/ACTIVE 等语义名或 `all` 序列化为线上状态值。
...
- CV-MKT-012 blog 发布不变量：status=2(PUBLISHED) ⇒ slug 非空（V-MKT-052 / E-MKT-31 STEP-MKT-03）
...
**family 失效协议（多实例权威口径）**：下表 key 是逻辑业务后缀；实际 JetCache key 为
`marketing:{family}:v{generation}:{business-key}`。`generation` 存于 Redis 共享键
`marketing:cache-generation:{family}`，写事务提交后用 `INCR` 原子递增。所有应用实例下次读立即切换新代；
并发回填仍写入读取前捕获的旧代，不会污染新代。旧代 key 按 60~300s TTL 自然回收，不扫描 Redis，
也不依赖单进程 key registry，因此重启和水平扩容不会复活旧 Hero。Redis 短时不可用时使用单实例单调递增的
fallback generation，恢复后取本地/远端较大值，TTL 是最终收敛上界（EC-MKT-002）。

| ID | key 模板 | TTL | 装载点 | 失效触发者（@CacheInvalidate + MQ content.invalidated） |
|---|---|---|---|---|
| CACHE-MKT-001 | `marketing:banners:v{generation}:{position-code\|all}:{locale}`（position-code=1/2/3） | 300s | E-MKT-01 | Banner 创建/编辑/删除/状态（TX-MKT-007~010）+ SCHED-MKT-01 窗口穿越（DEC-MKT-2） |
...
| ID | 周期 | 逻辑 |
|---|---|---|
| SCHED-MKT-01 营销定时投放/到期下线（FLOW-P15，ALIGN-008/009，TASK-060） | 每分钟 @Scheduled | ① huihao-redis 分布式锁 `onIdLock("sched:marketing-promo")`（多实例防双跑，拿不到锁直接跳过本 tick）② TX-MKT-029：RM-MKT-109 coupon 翻转（scheduled→active→expiring→expired，DEC-MKT-3 阈值 72h 配置）+ RM-MKT-126 flash 翻转（scheduled→active；active→ended 自动下线 s-761）③ 提交后：flash 任一翻转 → `invalidateFamily(FLASH)` 递增 Redis 共享代际 + MQ `content.invalidated {type:flash_sale_changed}` ④ RM-MKT-008 banner 窗口穿越检测（lastTick 持久于 Redis key `sched:marketing-promo:last-tick`）→ 任一穿越 → `invalidateFamily(BANNERS)` 递增 Redis 共享代际 + MQ `{type:banner_changed}`（**状态不翻转**，DEC-MKT-2）⑤ coupon 翻转不发 MQ（无缓存面） |
...
CREATE TABLE banner_translation (
  ...
  cta_link_secondary VARCHAR(512) NULL COMMENT '历史扩展列；当前管理 API 不编辑，链接默认取 Banner 主表',
  created_at DATETIME(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
  updated_at DATETIME(3)  NOT NULL DEFAULT CURRENT_TIMESTAMP(3) ON UPDATE CURRENT_TIMESTAMP(3),
  PRIMARY KEY (id),
...
> 备注：①`processed_event` 幂等表归 trading 域 DDL（本域 q.invalidate 消费者复用）；②上述 mock/demo 种子只在显式设置 `DEMO_SEED_ENABLED=true`（`dreamy.seed.demo-enabled=true`）时灌入，默认为 false，正式启动不创建或覆盖 Banner/营销/首页演示数据；contact/newsletter 始终为纯收集表，不灌样例；③首个管理员不使用公开固定密码，新环境必须显式提供 `DREAMY_BOOTSTRAP_ADMIN_EMAIL` 和 `DREAMY_BOOTSTRAP_ADMIN_PASSWORD`（密码至少 12 字符）；仅开启 demo seed 时允许本地演示凭据。④banner.clicks 列保留 0 起始（点击统计接入归后续 change，er 字段保真）。
