...
- **FUNC-023 登录与认证配置**：超管配置消费端登录方式开关（email/google/apple 均可开闭，允许全关，风险自担）、OTP 策略（长度4/6/8、有效期1-30min、重发间隔10-120s、最大尝试3-10）、解绑最小方式数 min_methods(1-3)、OAuth 凭据展示；保存写认证配置变更日志，配置供消费端登录读取。
