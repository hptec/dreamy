# design-ref: content_publish_invalidate

> 本实现单元域的 L2 设计宿主：`specs/design/catalog/`（七业务域聚合设计）。

## 关联任务
- **TASK-056**（new_feature）：实现业务流程 content_publish_invalidate → 设计章节见 `specs/design/catalog/task-allocation.yml` 中 TASK-056 条目
