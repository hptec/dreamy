...
## 关联任务
- **TASK-035**（modify_logic）：实现 tag_lifecycle 状态机（4 个转换） → 设计章节见 `specs/design/catalog/task-allocation.yml` 中 TASK-035 条目
- **TASK-036**（modify_logic）：实现 tag_dimension_lifecycle 状态机（1 个转换） → 设计章节见 `specs/design/catalog/task-allocation.yml` 中 TASK-036 条目
