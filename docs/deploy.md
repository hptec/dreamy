...
- **边缘网关**:`gateway` 容器(nginx)是唯一对外入口,**单端口按路径分流并终结 TLS**:`/` → store、`/admin/` → admin(网关剥离前缀)、**store 身份域 API(`/api/store/auth/`、`/api/store/account/`)→ server(Rust,18082)**、**admin API 走加密前缀 `/${ADMIN_API_SECRET}/api/...`(网关剥壳后按既有口径分流:身份域 → server,其余 → backend;secret 配于 `.env.deploy` 的 `ADMIN_API_SECRET`,前端构建注入同值——前端源码与 Rust/Java 代码零改动)**、其余 `/api/` 与 `/actuator/` → backend;旧裸路径 `/api/admin/*` 对外一律 404(防扫描器探测);backend/server 仅绑宿主回环,admin/store 不映射宿主端口。HTTPS 为硬要求(Google OIDC 等外部回调的公网 redirect_uri 仅认 https),证书挂载自 `nginx/certs/`。
- **双库拓扑**(server 引入后):同一 MySQL 实例内 `identity` 库(Java 既有,业务表 + 身份域回滚快照死表)与 `dreamy_server` 库(Rust 身份域 11 张表 + `operation_log`/`email_template`,启动幂等自举;生产切换前由 `scripts/migrate-identity.sh` + `scripts/migrate-shared-tables.sh` 在维护窗口做一次性数据迁移)共存。分流前缀矩阵由 `scripts/test-gateway-routes.sh` 门禁保障。
- **gRPC 内部通道(18083,仅 compose 内网)**:Java backend → Rust server 单向,三服务共用一端口——`IdentityGate`(会话校验/权限/用户)、`AuditGate`(业务侧审计写入/日志查询)、`TemplateGate`(邮件模板);契约见 `proto/dreamy/*/v1/`。审计/模板通道常开(不受 `IDENTITY_GRPC_ENABLED` 门控,回滚态仍工作;失败 best-effort/进邮件重试链)。
...
| ↑ 同端口 | `/{ADMIN_API_SECRET}/api/admin/auth/`、`auth-config`、`admins`、`roles`、`permissions`、`users`、`operation-logs`(网关剥壳) | server:18082 | admin 身份域 API(secret 段) |
| ↑ 同端口 | `/{ADMIN_API_SECRET}/api/` 其余(网关剥壳) | backend:18081 | admin 业务 API(secret 段) |
| ↑ 同端口 | `/api/admin`、`/api/admin/*` | —(404) | 旧裸路径对外关闭,防扫描器探测 |
...
- 网关分流前缀(store 身份域 + admin secret 段)与 `scripts/test-gateway-routes.sh` 路由矩阵互为门禁,改前缀必须同步改矩阵。
