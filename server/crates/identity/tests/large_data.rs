...
use identity::service::user_query::{self, Cond, Col, CondVal, ListQuery};
...
fn measure_p<N, F>(runs: usize, mut f: F) -> (f64, f64)
where
    F: FnMut() -> N,
{
...
fn assert_ms(label: &str, p50: f64, p95: f64, budget: f64) {
    println!("  {label:<38} p50={p50:8.2}ms  p95={p95:8.2}ms  (预算 {budget}ms)");
    assert!(
        p95 < budget,
        "{label} p95 {p95:.1}ms 超预算 {budget}ms——证据触发优化动作"
    );
}
...
async fn large_data_evidence() {
    ...
    let total: i64 = db
        ...
        .unwrap_or(0);
    println!("[large-data] 灌数完成:{total} 行,耗时 {:.1}s", t0.elapsed().as_secs_f64());
    ...
    let state: SharedState = std::sync::Arc::new(AppState {
        ...
        cfg: common::config::Config {
            ...
            db_host: String::new(), db_port: 0,
            db_name: "dreamy_bench".into(), db_legacy_name: String::new(),
            db_user: String::new(), db_password: String::new(),
            redis_host: String::new(), redis_port: 0,
            store_jwt_secret: None, admin_jwt_secret: None,
            store_cors_origin: String::new(), admin_cors_origin: String::new(),
            schema_path: String::new(),
        },
    });

    println!("[large-data] 延迟测量(50 次/场景,p50/p95):");

    // ① 首页:默认排序(created_at desc)+ id 兜底,page 1
    {
        let q = ListQuery { conds: vec![], order: (Col::CreatedAt, true), page: 1, page_size: 20 };
        let (p50, p95) = measure_p(50, || {
            futures::executor::block_on(user_query::list_users(&state, &q)).unwrap()
        });
        assert_ms("① 首页(created_at desc, 20 行)", p50, p95, 300.0);
    }

    // ② id 排序首页(主键有序,最优情形对照)
    {
        let q = ListQuery { conds: vec![], order: (Col::Id, true), page: 1, page_size: 20 };
        let (p50, p95) = measure_p(50, || {
            futures::executor::block_on(user_query::list_users(&state, &q)).unwrap()
        });
        assert_ms("② 首页(id desc, 20 行)", p50, p95, 50.0);
    }

    // ③ 深分页(中段页码)——已知风险点:created_at 无索引
    {
        let mid_page = (total as u64 / 20 / 2).max(1);
        let q = ListQuery { conds: vec![], order: (Col::CreatedAt, true), page: mid_page, page_size: 20 };
        let (p50, p95) = measure_p(10, || {
            futures::executor::block_on(user_query::list_users(&state, &q)).unwrap()
        });
        assert_ms("③ 深分页(created_at desc 中段)", p50, p95, 1500.0);
    }
    ...
    // ④ 前缀过滤(email 唯一索引可走)
    {
        let q = ListQuery {
            ...
            order: (Col::Id, false), page: 1, page_size: 20,
        };
        let (p50, p95) = measure_p(50, || {
            futures::executor::block_on(user_query::list_users(&state, &q)).unwrap()
        });
        assert_ms("④ Email 前缀过滤(uk 索引)", p50, p95, 100.0);
    }
    ...
    // ⑤ 状态过滤 + 计数(全表命中)
    {
        let q = ListQuery {
            ...
            order: (Col::Id, true), page: 1, page_size: 20,
        };
        let (p50, p95) = measure_p(10, || {
            futures::executor::block_on(user_query::list_users(&state, &q)).unwrap()
        });
        assert_ms("⑤ 状态过滤 + 全量计数(百万行)", p50, p95, 800.0);
    }
    ...
    // ⑥ GetUser 主键直查(无缓存,纯 DB)
    {
        ...
        let (p50, p95) = measure_p(50, || {
            futures::executor::block_on(user_query::get_user(&state, &lookup)).unwrap()
        });
        assert_ms("⑥ GetUser by id(主键)", p50, p95, 20.0);
    }
    ...
    // ⑦ GetUser by email(唯一索引)
    {
        ...
        assert_ms("⑦ GetUser by email(uk 索引)", p50, p95, 20.0);
    }
