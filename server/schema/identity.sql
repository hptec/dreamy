...
CREATE TABLE IF NOT EXISTS `auth_config` (
  ...
  `verify_ip_rate_per_minute` INT NOT NULL DEFAULT 30 COMMENT 'verify OTP IP 频控 5-300 次/分钟',
  `attack_alert_threshold`   INT NOT NULL DEFAULT 20 COMMENT 'verify 连续失败告警阈值 10-1000',
  `admin_alert_email`        VARCHAR(255) DEFAULT NULL COMMENT '安全告警通知邮箱(空=仅告警日志)',
  `store_access_ttl_minutes` INT NOT NULL DEFAULT 120 COMMENT 'store access token 有效期 10-1440 分钟',
  `store_refresh_ttl_days`   INT NOT NULL DEFAULT 30 COMMENT 'store refresh token 有效期 1-90 天',
  `admin_access_ttl_hours`   INT NOT NULL DEFAULT 8 COMMENT 'admin access token 有效期 1-24 小时',
