...
export default function CartPage() {
    ...
    return (
      <div className="container-luxe py-20">
        <div className="flex flex-col items-center gap-5 py-16 text-center">
          ...
          <h1 className="font-display text-4xl font-medium">{t.cart.page.emptyTitle}</h1>
          <p className="max-w-sm text-ink-soft">{t.cart.page.emptyBody}</p>
          <Link href="/wedding-dresses" className="btn-primary">{t.cart.page.shopDresses}</Link>
        </div>
        {recommended.length > 0 && (
          <section className="mt-16">
            <h2 className="mb-8 text-center font-display text-3xl font-medium">{t.cart.page.bestSellers}</h2>
  ...
  return (
    <div className="container-luxe py-12">
      <h1 className="font-display text-4xl font-medium">{t.cart.page.title}</h1>
      {truncatedItemIds.length > 0 && (
        <p className="mt-4 rounded-sm bg-sage/10 px-4 py-3 text-sm text-sage-deep">{t.cart.page.mergedNotice}</p>
      )}
      <div className="mt-8 grid gap-12 lg:grid-cols-3">
              ...
              <div className={cn('flex gap-5 border-b border-line py-6', line.unavailable && 'opacity-50')}>
                ...
                <div className="flex flex-1 flex-col">
                  ...
                  <p className="mt-1 text-sm text-ink-soft">
                    {[line.color, line.customSizeData ? t.cart.drawer.customSize : line.size ? t.cart.page.sizeLabel.replace('{size}', line.size) : undefined].filter(Boolean).join(' · ')}
                  </p>
                  {line.customSizeData && (
                    <p className="mt-0.5 text-xs text-ink-faint">
                      {t.cart.page.bust} {line.customSizeData.bust}″ · {t.cart.page.waist} {line.customSizeData.waist}″ · {t.cart.page.hips} {line.customSizeData.hips}″ · {t.cart.page.hollowToFloor} {line.customSizeData.hollowToFloor}″
                    </p>
                  )}
                  {line.unavailable && <p className="mt-1 text-xs text-blush">{t.cart.page.unavailable}</p>}
                  ...
                  <div className="mt-auto flex items-center gap-4">
                    ...
                    >
                      <Heart className="h-3.5 w-3.5" /> {t.cart.page.saveForLater}
                    </button>
                    <button onClick={() => void removeLine(line.key)} className="flex cursor-pointer items-center gap-1 text-xs text-ink-soft hover:text-blush"><X className="h-3.5 w-3.5" /> {t.cart.drawer.remove}</button>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div>
          <div className="rounded-sm border border-line bg-surface p-6 lg:sticky lg:top-28">
            <h2 className="font-display text-2xl font-medium">{t.checkout.orderSummary}</h2>
            <dl className="mt-5 space-y-2.5 border-t border-line pt-5 text-sm">
              <div className="flex justify-between"><dt className="text-ink-soft">{t.cart.page.subtotal}</dt><dd>{formatPrice(cartSubtotal, currency)}</dd></div>
              <div className="flex justify-between"><dt className="text-ink-soft">{t.cart.page.shipping}</dt><dd className="text-ink-soft">{t.cart.page.calculatedAtCheckout}</dd></div>
              <div className="flex justify-between border-t border-line pt-3 text-base font-medium"><dt>{t.cart.page.estimatedTotal}</dt><dd className="font-display text-xl">{formatPrice(cartSubtotal, currency)}</dd></div>
            </dl>
            <p className="mt-2 text-xs text-ink-faint">{t.cart.drawer.installments.replace('{amount}', formatPrice(cartSubtotal / 4, currency))}</p>
            <p className="mt-2 text-xs text-ink-faint">{t.cart.page.promoNote}</p>
            <Link href="/checkout" className="btn-primary mt-5 w-full">{t.cart.page.checkoutCta}</Link>
            <div className="mt-5 space-y-2 text-xs text-ink-soft">
              <p className="flex items-center gap-2"><Truck className="h-4 w-4 text-gold" /> {t.cart.page.freeShipping}</p>
              <p className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-gold" /> {t.cart.page.secureCheckout}</p>
            </div>
          </div>
        </div>
      </div>
