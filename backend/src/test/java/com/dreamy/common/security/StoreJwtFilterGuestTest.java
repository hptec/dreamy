...
import com.dreamy.security.StoreSecurityProperties;
...
class StoreJwtFilterGuestTest {
    ...
    /** store token + jti 对(测试内构造;与 Rust issue_store claims 同构——签发权在 Rust) */
    private record StoreToken(String token, String tokenId) {
    }

    private String buildAdminToken(String adminId, String roleId) {
        LocalDateTime now = LocalDateTime.now(ZoneOffset.UTC);
        SecretKey key = Keys.hmacShaKeyFor(
                props.getAdmin().getSecret().getBytes(StandardCharsets.UTF_8));
        return Jwts.builder()
                .subject(adminId)
    ...
    private StoreToken buildStoreToken(String userId, String method, boolean refresh) {
        LocalDateTime now = LocalDateTime.now(ZoneOffset.UTC);
        String jti = UUID.randomUUID().toString();
        SecretKey key = Keys.hmacShaKeyFor(
                props.getStore().getSecret().getBytes(StandardCharsets.UTF_8));
        String token = Jwts.builder()
                ...
                .signWith(key)
                .compact();
        return new StoreToken(token, jti);
    }
    ...
    @Test
    @DisplayName("公开白名单路径无 token 放行（回归保持）")
    void legacyPublicPathAnonymousPass() throws Exception {
        MockHttpServletResponse response = run("GET", "/api/store/products/123", null);
    ...
    void publicPathOptionalPrincipalInjection() throws Exception {
        StoreToken pair = buildStoreToken("user-1", "email", false);
        when(sessionValidator.isStoreSessionValid(pair.tokenId())).thenReturn(true);
    ...
    void storeTokenSessionValidation() throws Exception {
        StoreToken pair = buildStoreToken("user-2", "email", false);
        when(sessionValidator.isStoreSessionValid(pair.tokenId())).thenReturn(true);
        AtomicReference<AuthPrincipal> seen = new AtomicReference<>();
        MockHttpServletResponse ok = run("GET", "/api/store/cart", pair.token(),
                (req, res) -> seen.set(AuthContext.get()));
        assertThat(ok.getStatus()).isEqualTo(200);
        assertThat(seen.get().type()).isEqualTo(AuthPrincipal.TYPE_STORE);

        when(sessionValidator.isStoreSessionValid(pair.tokenId())).thenReturn(false);
        MockHttpServletResponse revoked = run("GET", "/api/store/cart", pair.token());
    ...
    void crossEndAdminTokenRejected() throws Exception {
        MockHttpServletResponse response = run("GET", "/api/store/cart", buildAdminToken("admin-1", "role-1"));
        assertThat(response.getStatus()).isEqualTo(401);
        assertThat(response.getContentAsString()).contains("\"code\":40100");
    }
}
