...
/**
 * catalog 域操作审计记录器（BE-DIM-7）。
 * 复用 identity AuditService（operation_log 只读不可删）；与 identity AOP 切面的差异：
 * 本域审计行在事务内写入（api-detail STEP「INSERT operation_log」属 TX-CAT-* 边界）且携带
 * changes before/after JSON（flags/上下架/标签状态流转）。本域 19 个 action 枚举见 api-detail §0。
 */
@Component
public class CatalogAuditRecorder {

    private static final Logger log = LoggerFactory.getLogger(CatalogAuditRecorder.class);

    private final AuditService auditService;
    private final AdminUserMapper adminUserMapper;

    public CatalogAuditRecorder(AuditService auditService, AdminUserMapper adminUserMapper) {
    ...
    public void record(String action, String target, String changesJson) {
        try {
            ...
            log.warn("[AUDIT-CAT] record failed action={} target={}", action, target, ex);
        }
    }
