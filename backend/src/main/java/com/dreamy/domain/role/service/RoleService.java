package com.dreamy.domain.role.service;
...
public class RoleService {
    ...
    public com.dreamy.dto.RoleDTO toRoleDTO(Role role) {
        List<String> keys = effectivePermissionKeys(role);
        long memberCount = countMembers(role.getId());
        return new com.dreamy.dto.RoleDTO(role.getId(), role.getName(),
                role.getType(), role.getIsLocked(), memberCount, keys);
    }

    /** listRoles DTO 视图 */
    public List<com.dreamy.dto.RoleDTO> listRoleDTOs() {
        return listRoles().stream().map(this::toRoleDTO).toList();
    }

    /** createRole 后返回 DTO */
    public com.dreamy.dto.RoleDTO createRoleDTO(String name) {
        return toRoleDTO(createRole(name));
    }

    /** updateRole 后返回 DTO */
    public com.dreamy.dto.RoleDTO updateRoleDTO(Long roleId, String name, List<String> permissionKeys) {
