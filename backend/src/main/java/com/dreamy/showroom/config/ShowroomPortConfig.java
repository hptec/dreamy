package com.dreamy.showroom.config;
...
import com.dreamy.catalog.domain.product.repository.ProductImageMapper;
import com.dreamy.catalog.domain.product.repository.ProductMapper;
import com.dreamy.catalog.domain.product.repository.ProductTranslationMapper;
 ...
 * - CatalogSnapshotPort：基于 catalog 既有 Mapper 的只读适配（name 按 locale 经 product_translation
 *   回退解析——决策 13；image_url 取 gallery sort 最小为主图，CV-CAT-010 口径；
 *   提供方后续给出真实 bean 自动让位，review/marketing 同范式）。
 * - IdentityQueryPort：基于 identity UserMapper 的只读适配。
...
public class ShowroomPortConfig {
    ...
    public CatalogSnapshotPort showroomCatalogSnapshotPortAdapter(ProductMapper productMapper,
                                                                  ProductTranslationMapper translationMapper,
                                                                  ProductImageMapper imageMapper) {
        return (productIds, locale) -> {
            Map<Long, CatalogSnapshotPort.ProductCardBrief> result = new LinkedHashMap<>();
            ...
            for (Product product : productMapper.selectList(
                ...
                result.put(product.getId(), new CatalogSnapshotPort.ProductCardBrief(
    ...
    public IdentityQueryPort showroomIdentityQueryPortAdapter(UserMapper userMapper) {
        return customerId -> {
            if (customerId == null) {
                return null;
            }
