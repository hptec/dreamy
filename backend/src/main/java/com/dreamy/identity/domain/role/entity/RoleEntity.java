...
public class RoleEntity extends LongAuditableEntity {
    ...
    @Column(name = "name", definition = "varchar(64) NOT NULL COMMENT '角色名'")
    private String name;

    /** type: preset/custom（ck_role_type） */
    @Column(name = "type", definition = "varchar(16) NOT NULL DEFAULT 'custom' COMMENT '类型 preset/custom'")
    private String type;

    @Column(name = "is_locked", definition = "tinyint(1) NOT NULL DEFAULT 0 COMMENT '锁定（超管保护）'")
