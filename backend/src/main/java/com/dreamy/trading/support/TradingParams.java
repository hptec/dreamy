...
import com.dreamy.trading.dto.TradingDtos.CustomSizeData;
 ...
 * 非法 → FieldErrors 收集 422601）。枚举取值与契约逐字一致（TC-TRD-052）。
 */
public final class TradingParams {
    ...
    public static String requireText(String value, int maxLength, String field, FieldErrors errors) {
     ...
     * （CV-TRD-006）。
     */
    public static void validateCustomSize(CustomSizeData data, FieldErrors errors) {
        if (data == null) {
            return;
        }
    ...
    private static void requireNonNegative(BigDecimal value, String field, FieldErrors errors) {
        if (value == null) {
            errors.reject(field, "required");
