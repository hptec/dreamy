package com.dreamy.trading.config;
...
import com.dreamy.catalog.domain.product.repository.SkuMapper;
 ...
 * - CatalogSnapshotPort：基于 catalog 既有 Mapper/Repository 的只读适配（catalog 域后续提供
 ...
 * - DyeLotPort：showroom 域并行实现中，本域 stub 兜底（恒空数组——getCart.STEP-TRD-03 口径，决策 20.4）。
 */
@Configuration
public class TradingPortConfig {
    ...
    @Bean
    @ConditionalOnMissingBean(DyeLotPort.class)
    public DyeLotPort stubDyeLotPort() {
        log.info("[TRADING] DyeLotPort stub active (showroom 域未就绪，dye_lot_product_ids 恒空)");
        return (customerId, productIds) -> List.of();
    }

    @Bean
    @ConditionalOnMissingBean(CatalogSnapshotPort.class)
    public CatalogSnapshotPort tradingCatalogSnapshotPortAdapter(ProductMapper productMapper,
