...
class GlobalExceptionHandlerTest {
    ...
    void handleBiz_returnsCorrectEnvelope() {
        ...
        // ACT
        ResponseEntity<ErrorBody> resp = handler.handleBiz(ex);

        // ASSERT: L1 HTTP 401, L2 code=40101, L3 message 存在, L4 details.remaining_attempts=3
        ...
        assertThat(resp.getBody().getDetails()).containsEntry("remaining_attempts", 3);
    }
    ...
    void handleInfra_returns5xxWithoutDetails() {
        ...
        // ACT
        ResponseEntity<ErrorBody> resp = handler.handleInfra(ex);

        // ASSERT: L1 HTTP 500, L2 code=50001, L3 details=null（不泄漏）
        assertThat(resp.getStatusCode().value()).isEqualTo(500);
        assertThat(resp.getBody().getCode()).isEqualTo(50001);
        assertThat(resp.getBody().getDetails()).isNull();
    }
    ...
    void handleUnexpected_returns50000() {
        ...
        // ACT
        ResponseEntity<ErrorBody> resp = handler.handleUnexpected(new RuntimeException("boom"));

        // ASSERT: L1 HTTP 500, L2 code=50000, L3 details=null
        assertThat(resp.getStatusCode().value()).isEqualTo(500);
