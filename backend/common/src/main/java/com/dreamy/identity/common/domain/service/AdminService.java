...
import com.dreamy.identity.common.infra.AdminSessionValidityCache;
...
public class AdminService {
    ...
    public LoginOutcome login(String email, String password, String ip, String device) {
        ...
        JwtTokenProvider.AdminToken token = jwtTokenProvider.issueAdminToken(
                admin.getId(), admin.getRoleId(), permissionKeys);

        OffsetDateTime now = OffsetDateTime.now(ZoneOffset.UTC);
        AdminSessionEntity session = new AdminSessionEntity();
        session.setId(IdGenerator.uuid());
    ...
    public AdminUserEntity requireActiveAdmin(String adminId, String tokenId) {
    ...
    public AdminUserEntity findById(String id) {
    ...
    public IPage<AdminUserEntity> pageAdmins(int page, int pageSize, String status, String roleId) {
        LambdaQueryWrapper<AdminUserEntity> qw = new LambdaQueryWrapper<>();
        if (status != null) {
    ...
    @Transactional
    public AdminUserEntity createAdmin(String name, String email, String password, String roleId) {
    ...
    @Transactional
    public AdminUserEntity updateAdmin(String id, String name, String roleId) {
    ...
    @Transactional
    public AdminUserEntity toggleStatus(String id, String status) {
        AdminUserEntity admin = requireExist(id);
        if (isSuperAdmin(admin)) {
            throw new BizException(ErrorCode.SUPER_ADMIN_PROTECTED); // 40306
        }
    ...
    private void revokeAdminSessions(String adminId) {
        LambdaQueryWrapper<AdminSessionEntity> qw = new LambdaQueryWrapper<>();
    ...
    private AdminUserEntity requireExist(String id) {
    ...
    /** adminMe 数据组装（role_name + is_super） */
    public MeData meData(String adminId, String tokenId) {
        AdminUserEntity admin = requireActiveAdmin(adminId, tokenId);
        RoleEntity role = roleMapper.selectById(admin.getRoleId());
        boolean isSuper = role != null && Boolean.TRUE.equals(role.getIsLocked());
        return new MeData(toAdminDTO(admin), role != null ? role.getName() : "", isSuper);
    }

    /** 分页管理员 DTO（含 total/分页元数据） */
    public PageData<AdminDTO> pageAdminDTOs(int page, int pageSize, String status, String roleId) {
    ...
    public AdminDTO createAdminDTO(String name, String email, String password, String roleId) {
        return toAdminDTO(createAdmin(name, email, password, roleId));
    }

    public AdminDTO updateAdminDTO(String id, String name, String roleId) {
        return toAdminDTO(updateAdmin(id, name, roleId));
    }

    public AdminDTO toggleStatusDTO(String id, String status) {
        return toAdminDTO(toggleStatus(id, status));
    }

    public record MeData(AdminDTO admin, String roleName, boolean isSuper) {}
