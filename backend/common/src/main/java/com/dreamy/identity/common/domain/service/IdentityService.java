...
import com.dreamy.identity.common.infra.oidc.OidcResult;
...
public class IdentityService {
    ...
    private final com.dreamy.identity.common.dto.mapper.IdentityDtoMapper dtoMapper;
    ...
    public IdentityService(UserMapper userMapper,
                           ...
                           com.dreamy.identity.common.dto.mapper.IdentityDtoMapper dtoMapper) {
    ...
    public com.dreamy.identity.common.dto.UserProfileDTO getProfileView(String userId) {
        return dtoMapper.toProfile(getProfile(userId));
    }

    /** FUNC-010 表示层：登录方式 DTO 列表（MAP-002 隐藏 provider_uid） */
    public List<com.dreamy.identity.common.dto.IdentityDTO> listIdentityViews(String userId) {
        return listIdentities(userId).stream().map(dtoMapper::toIdentity).toList();
    }

    /** FLOW-05 表示层：绑定后返回最新登录方式 DTO 列表 */
    public List<com.dreamy.identity.common.dto.IdentityDTO> bindIdentityViews(
            String userId, String provider, String idToken, String email, String code, String ip) {
    ...
    public List<com.dreamy.identity.common.dto.IdentityDTO> changePrimaryEmailViews(
            String userId, String newEmail, String code) {
     ...
     */
    @Transactional
    public LoginResult oidcLogin(String provider, String idToken, String nonce, LoginContext ctx) {
        // V-006 provider 开关
        AuthConfigEntity cfg = authConfigService.getConfig();
        if ("google".equals(provider) && !Boolean.TRUE.equals(cfg.getGoogleEnabled())) {
            throw new BizException(ErrorCode.PROVIDER_DISABLED); // 40303
        }
        if ("apple".equals(provider) && !Boolean.TRUE.equals(cfg.getAppleEnabled())) {
            throw new BizException(ErrorCode.PROVIDER_DISABLED); // 40303
        }
        // STEP-01 OIDC 验证（50401/50201 由 verifier 抛 InfraException）
        OidcResult oidc = oidcVerifier.verify(provider, idToken, nonce);
        ...
        MergeService.MergeOutcome outcome = mergeService.resolveOrMerge(
                ...
                oidc.hiddenEmail(), oidc.relayEmail());
    ...
    @Cached(name = "store:identities:", key = "#userId", cacheType = CacheType.BOTH,
            expire = 300, localExpire = 300)
    public List<UserIdentityEntity> listIdentities(String userId) {
        LambdaQueryWrapper<UserIdentityEntity> qw = new LambdaQueryWrapper<>();
        qw.eq(UserIdentityEntity::getUserId, userId)
                .eq(UserIdentityEntity::getConnected, true);
        return identityMapper.selectList(qw);
    }

    /** RM-011：用户全部凭证（不过滤 connected，用于 admin 详情/计数） */
    public List<UserIdentityEntity> listAllIdentities(String userId) {
        LambdaQueryWrapper<UserIdentityEntity> qw = new LambdaQueryWrapper<>();
        qw.eq(UserIdentityEntity::getUserId, userId);
        return identityMapper.selectList(qw);
    }
    ...
    @Transactional
    public List<UserIdentityEntity> bindIdentity(String userId, String provider, String idToken,
        ...
        if ("email".equals(provider)) {
            ...
            OidcResult oidc = oidcVerifier.verify(provider, idToken, null);
        ...
        // STEP-02 占用校验
        UserIdentityEntity occupied = mergeService.findByProviderUid(provider, providerUid);
        ...
        if (occupied != null) {
            // STEP-03 重新连接既有凭证
            LambdaUpdateWrapper<UserIdentityEntity> uw = new LambdaUpdateWrapper<>();
            uw.eq(UserIdentityEntity::getId, occupied.getId())
                    .set(UserIdentityEntity::getConnected, true)
                    .set(UserIdentityEntity::getLastLoginAt, now);
            identityMapper.update(null, uw);
        } else {
            UserIdentityEntity identity = new UserIdentityEntity();
            identity.setId(com.dreamy.identity.common.util.IdGenerator.uuid());
    ...
    public void unbindIdentity(String userId, String identityId) {
        UserIdentityEntity identity = identityMapper.selectById(identityId);
        // STEP-01 归属
        ...
        LambdaUpdateWrapper<UserIdentityEntity> uw = new LambdaUpdateWrapper<>();
        uw.eq(UserIdentityEntity::getId, identityId)
    ...
    @CacheInvalidate(name = "store:identities:", key = "#userId")
    @Transactional
    public List<UserIdentityEntity> changePrimaryEmail(String userId, String newEmail, String code) {
        ...
        LambdaUpdateWrapper<UserIdentityEntity> demote = new LambdaUpdateWrapper<>();
        demote.eq(UserIdentityEntity::getUserId, userId)
                .eq(UserIdentityEntity::getIsPrimary, true)
                .set(UserIdentityEntity::getIsPrimary, false);
        identityMapper.update(null, demote);

        // 新邮箱凭证 is_primary=true（不存在则建已验证 email 凭证）
        UserIdentityEntity newPrimary = findEmailIdentity(userId, normalized);
        if (newPrimary == null) {
            newPrimary = new UserIdentityEntity();
            newPrimary.setId(com.dreamy.identity.common.util.IdGenerator.uuid());
            newPrimary.setUserId(userId);
            newPrimary.setProvider("email");
            ...
            newPrimary.setCreatedAt(now);
            newPrimary.setUpdatedAt(now);
            identityMapper.insert(newPrimary);
        } else {
            LambdaUpdateWrapper<UserIdentityEntity> promote = new LambdaUpdateWrapper<>();
            promote.eq(UserIdentityEntity::getId, newPrimary.getId())
                    .set(UserIdentityEntity::getIsPrimary, true)
                    .set(UserIdentityEntity::getVerified, true);
    ...
    public void deleteAccount(String userId) {
        ...
        uw.eq(UserEntity::getId, userId)
                ...
                .set(UserEntity::getDeletedAt, now);
    ...
    private List<UserIdentityEntity> listConnected(String userId) {
        LambdaQueryWrapper<UserIdentityEntity> qw = new LambdaQueryWrapper<>();
        qw.eq(UserIdentityEntity::getUserId, userId)
                .eq(UserIdentityEntity::getConnected, true);
        return identityMapper.selectList(qw);
    ...
    public long countConnected(String userId) {
        LambdaQueryWrapper<UserIdentityEntity> qw = new LambdaQueryWrapper<>();
        qw.eq(UserIdentityEntity::getUserId, userId)
                .eq(UserIdentityEntity::getConnected, true);
    ...
    private UserIdentityEntity findEmailIdentity(String userId, String email) {
        LambdaQueryWrapper<UserIdentityEntity> qw = new LambdaQueryWrapper<>();
        qw.eq(UserIdentityEntity::getUserId, userId)
