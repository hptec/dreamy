...
import com.dreamy.identity.common.config.CommonConfig;
...
import com.dreamy.identity.store.config.StoreConfig;
import com.dreamy.identity.store.filter.StoreJwtFilter;
...
class StoreAuthControllerTest {
    ...
    void getAuthConfig_noToken_returns200() throws Exception {
        ...
        when(authConfigService.getConfigView()).thenReturn(
                new com.dreamy.identity.common.dto.AuthConfigView(
                        true, true, false, 6, 10, 60, 5, 2, null, null));

        // ACT + ASSERT: L1 HTTP 200, L2 code 字段存在, L3 结构完整
        mockMvc.perform(get("/api/store/auth/config"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.email_enabled").value(true))
                .andExpect(jsonPath("$.google_enabled").value(true))
                .andExpect(jsonPath("$.otp_length").value(6));
    }
    ...
    void sendOtp_noToken_returns200() throws Exception {
        ...
        // ACT + ASSERT: L1 200, L2 resend_after_seconds, L3 otp_length
    ...
    void verifyOtp_noToken_returns200() throws Exception {
        // ARRANGE
        UserEntity user = activeUser("user-1", "user@example.com");
        TokenPair tokens = buildTokenPair();
        when(mapper.toProfile(any(UserEntity.class))).thenReturn(
                new UserProfileDTO("user-1", "user@example.com", true, null, null, "regular", null, null));
        ...
        // ACT + ASSERT: L1 200, L2 tokens 存在, L3 user/is_new_account
        mockMvc.perform(post("/api/store/auth/otp/verify")
                ...
                .andExpect(jsonPath("$.is_new_account").value(false));
    }
    ...
    void refresh_validRefreshToken_returns200() throws Exception {
        ...
        // ACT + ASSERT: L1 200, L2 tokens 存在
        mockMvc.perform(post("/api/store/auth/refresh")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"refreshToken\":\"" + original.getRefreshToken() + "\"}"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.tokens.access_token").isNotEmpty());
    }
    ...
    void accountProfile_withStoreToken_passesFilter() throws Exception {
        ...
        when(identityService.getProfileView("user-1")).thenReturn(
                new UserProfileDTO("user-1", "user@example.com", true, null, null, "regular", null, null));

        // ACT + ASSERT: L1 200, L2 user.id
        mockMvc.perform(get("/api/store/account/profile")
    ...
    void verifyOtp_wrongCode_returns401WithDetails() throws Exception {
        ...
        // ACT + ASSERT: L1 401, L2 code=40101 + message, L3 details.remaining_attempts, L4 message 英文
        mockMvc.perform(post("/api/store/auth/otp/verify")
                        .contentType(MediaType.APPLICATION_JSON)
                ...
                .andExpect(jsonPath("$.details.remaining_attempts").value(3));
    }
    ...
    void sendOtp_invalidEmail_returns422() throws Exception {
        // ASSERT: L1 422, L2 code=40000, L3 details.email 存在
        mockMvc.perform(post("/api/store/auth/otp/send")
                        .contentType(MediaType.APPLICATION_JSON)
    ...
    private UserEntity activeUser(String id, String email) {
